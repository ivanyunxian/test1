<template>
  <div>
    ...
    <a-card v-if="flag == 'personal'" class="card"  :bordered="false" >
    ...
    <a-row>
      <a-col :lg="24" :md="24" :sm="24">
        <a-form-item :style="{ textAlign: 'center' }">
          <a-button  style="margin-right: 8px" v-if="flag == 'parcel'" @click="selectzd">添加单元</a-button>
...
<script>
  ...
  import UserApplyTemplateModal from '@views/enterprise/register/modules/UserApplyTemplateModal'
  import ZdbdcListModal from '@views/enterprise/register//modules/ZdbdcListModal'
    export default {
      name: "Step1_AddParcel",
      ...
      data () {
        return {
          ...
          url: {
            ...
            deleteBatch: "/sys/user/enterprise/deleteBatch",
            exportXlsUrl: "message/sysMessageTemplate/exportXls",
            importExcelUrl: "message/sysMessageTemplate/importExcel",
            getuserbyenterprise:'/sys/user/enterprise/queryuserbyenterpriseid',
            selectedzdlist : "/modules/yspt/shyqzd/queryByEnterpriseId",
        ...
        submitProject () {
          ...
          this.$confirm({
            ...
            onOk: function() {
              ...
              if(that.dataSource.length == 0 && that.flag == 'personal'){
              ...
              if(that.selectedZddata.length == 0 && that.flag == 'parcel'){
                that.$notification['error']({
                  message: '请先添加宗地',
                  description: ""
                })
              ...
              var type = "";
              if(that.flag == 'personal'){
                type = "2";
              }else if(that.flag == 'parcel'){
                 type = "3";
              }
              ajaxaciton.get(window._CONFIG['domianURL'] + that.url.submitProject, { enterpriseid: that.enterpriseid,type:type }, function(response) {
                  if (response) {
                    if (response.success) {
                      that.nextStep()
                    } else {
