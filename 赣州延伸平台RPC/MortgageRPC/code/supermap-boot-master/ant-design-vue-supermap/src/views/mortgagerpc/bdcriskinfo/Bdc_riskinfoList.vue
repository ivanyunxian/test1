          ...
          <a-col :md="6" :sm="8">
            <a-form-item
              ...
              <a-range-picker
                ...
                v-decorator="[
                  'slsj',
                  {rules: [{ required: false, message: '请选择受理期限'}]}
                ]"
              />
            </a-form-item>
          </a-col>
...
<script>
        ...
        columns: [
          {
           ...
           },
		   {
	    	...
	    	url: {
          list: "/modules/bdc_riskinfo/getLimitUnitInfo",
       }
    }
  },
  computed: {
  ...
  methods: {
    ...
    searchdata(arg) {
      ...
      //转换一下时间格式
      var slsj = this.form.getFieldValue('slsj');
      if(slsj) {
        var slsj_q = slsj[0].format('YYYY-MM-DD');
        var slsj_z = slsj[1].format('YYYY-MM-DD');
        params.slsj_q = slsj_q;
        params.slsj_z = slsj_z;
      }
      this.loading = true;
