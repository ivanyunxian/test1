...
public class SysDepartController {
    ...
    /**
     * 查询数据 查出用户所属的所有部门,并以树结构数据格式响应给前端
     *
     * @return
     */
    @RequestMapping(value = "/queryDepartByUserTreeList", method = RequestMethod.GET)
    public Result<List<SysDepartTreeModel>> queryDepartByUserTreeList(HttpServletRequest request) {
        Result<List<SysDepartTreeModel>> result = new Result<>();
		// 获取登录用户信息
		LoginUser sysUser = (LoginUser) SecurityUtils.getSubject().getPrincipal();
        try {
            List<SysDepartTreeModel> list = new ArrayList<>();
			// 如果当前登录部门类型是登记中心或系统运维，查询所有部门
			...
			if ("0".equals(sysDepart.getDeptType()) || "1".equals(sysDepart.getDeptType())) {
                ...
                list = sysDepartService.queryDepartById(sysUser.getDeptId());
            }
            result.setResult(list);
            result.setSuccess(true);
        } catch (Exception e) {
   ...
   public Result<SysDepart> delete(@RequestParam(name="id",required=true) String id) {
	...
	public Result<List<DepartIdModel>> queryDepartTree() {
		...
		List<DepartIdModel> idList;
		try {
			boolean b = sysBaseAPI.checkPermission();
			if (!b) {
				// 获取登录用户信息
				LoginUser sysUser = (LoginUser) SecurityUtils.getSubject().getPrincipal();
				sysDepartService.queryTreeList(sysUser.getDeptId());
			} else {
				sysDepartService.queryTreeList();
			}
			idList = FindsDepartsChildrenUtil.wrapDepartIdModel();
	...
	public Result<List<SysDepartTreeModel>> searchBy(@RequestParam(name = "keyWord", required = true) String keyWord) {
		...
		try {
			List<SysDepartTreeModel> treeList = null;
			boolean b = sysBaseAPI.checkPermission();
			if (!b) {
				// 获取登录用户信息
				LoginUser sysUser = (LoginUser) SecurityUtils.getSubject().getPrincipal();
				treeList = this.sysDepartService.searhBy(keyWord, sysUser.getDeptId());
			} else {
				treeList = this.sysDepartService.searhBy(keyWord);
			}
