...
public class LoginController {
	...
	public Result<JSONObject> login(@RequestBody SysLoginModel sysLoginModel) throws Exception {
		...
		String socketstr = sysLoginModel.getStaff_uuid();
		...
		//用户登录信息
		if("1".equals(sysUser.getStartUkey())){
			if(socketstr.indexOf(";")>0){
				String[] licenseArr = socketstr.split(";");
				if (licenseArr.length < 2) {
					
				} else {
					String userDesStr = licenseArr[1].trim();
					String Key = "QsHdh18wnM94Edsp";
					String IV = "aVhjkf70tyIQ58PR";
					String userStr = AesEncryptUtil.desEncrypt(userDesStr, Key, IV);
					String[] userStrArr = userStr.split("\\|");
					for (int i = 0; i < userStrArr.length; i++) {
						if (userStrArr[i].startsWith("dj=")) {
							String[] djUserArr = userStrArr[i].split(";");
							if (djUserArr.length > 2) {
								// 验证用户名和UUID一致
								if (djUserArr[0].replace("dj=", "").equals(sysUser.getUsername())
										&& djUserArr[1].equals(sysUser.getStaffGuid())) {
									userInfo(sysUser, result);
									sysBaseAPI.addLog("用户名: " + username + ",登录成功！", CommonConstant.LOG_TYPE_1, null);
									return result;
								}else{
									result.setMessage("加密狗与用户不匹配");
	...
	public Result<?> selectDepart(@RequestBody SysUser user) {
		...
		this.sysUserService.updateUserDepart(username, orgCode, deptId);
		return Result.ok();
	}
	...
	private Result<JSONObject> userInfo(SysUser sysUser, Result<JSONObject> result) {
		...
		if (departs == null || departs.size() == 0) {
			...
			sysUserService.updateUserDepart(username, departs.get(0).getOrgCode(), departs.get(0).getId());
