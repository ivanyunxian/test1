...
public class SysUserServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements ISysUserService {
	...
	@Autowired
	private SysUserZrzMapper sysUserZrzMapper;
...
//		info.setSysUserName(user.getRealname());
		...
		List<String> sysMultiDeptId = new ArrayList<String>();
		if(list==null || list.size()==0) {
			//当前用户无部门
			//sysMultiOrgCode.add("0");
		}else if(list.size()==1) {
			sysMultiOrgCode.add(list.get(0).getOrgCode());
			sysMultiDeptId.add(list.get(0).getId());
		}else {
			info.setOneDepart(false);
			for (SysDepart dpt : list) {
				sysMultiOrgCode.add(dpt.getOrgCode());
				sysMultiDeptId.add(dpt.getId());
			}
		}
		info.setSysMultiOrgCode(sysMultiOrgCode);
		info.setSysMultiDeptId(sysMultiDeptId);
	...
	@Override
	public void updateUserDepart(String username,String orgCode, String deptId) {
		baseMapper.updateUserDepart(username, orgCode, deptId);
	}
...
//		this.save(user);  //保存角色的时候已经添加过一次了
	 ...
	 */
	@Override
	public Result<?> checkUserIsEffectiveNoLog(SysUser sysUser) {
		Result<?> result = new Result<Object>();
		//情况1：根据用户信息查询，该用户不存在
		if (sysUser == null) {
			result.error500("该用户不存在，请注册");
	...
	public IPage<SysUser> getUserByDepIdAll(Page<SysUser> page, String deptId) {
		return userMapper.getUserByDepIdAll(page, deptId);
	}


	@Override
	public IPage<SysUser> getUserByZrzId(Page<SysUser> page, String zrzId, String realname) {
		return userMapper.getUserByZrzId(page, zrzId,realname);
	}
}
