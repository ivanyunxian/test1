 ...
 * 转移登记+抵押权抵押人转移登记操作类
 * 
 * @ClassName: BG_YGandDYQBG_DJ_MulHandler
 * @author wuz
 ...
 *
 */
public class BG_YGandDYQBG_DJ_MulHandler extends DJHandlerBase implements DJHandler {
	 ...
	 * @param info
	 */
	public BG_YGandDYQBG_DJ_MulHandler(ProjectInfo info) {
		super(info);
		workflowcode=info.getBaseworkflowcode();
	}
	...
	public boolean addBDCDY(String bdcdyids) {
		...
		/*	StringBuilder builer = new StringBuilder();
		   ...
		   // dao.save(fsql);
			...
			// 将该不动产单元的权利人和抵押人拷贝到申请人（抵押权义务人为该单元的所有权人不用带，该单元所有权的义务人带不了）
			String fulsql = " SELECT QLID,QLLX FROM BDCK.BDCS_QL_XZ QL LEFT JOIN BDCK.BDCS_DJDY_XZ DJDY ON QL.DJDYID=DJDY.DJDYID  WHERE DJDY.BDCDYID='"
					+ bdcdyid + "' AND QL.QLLX in('23','"+super.getQllx().Value+"')";
			...
			if (listmap != null && listmap.size() > 0) {
				for (Map mp : listmap) {
					...
					if (!StringHelper.isEmpty(qlid)) {
						...
						//把最基本抵押权信息和主权利信息拷贝到工作层（变更前的单元主要功能是显示，都是去现状层读取数据的，只把基本的DJDY表、权力表拷贝到工作层为后面准备就行了）
				...
				if (djdy != null) {
					...
					dao.save(ql);
					dao.save(fsql);
					if(workflowcode.toUpperCase().equals("BG201")){
					dao.save(dyql);
					dao.save(dyfsql);
					}
			}
	...
	protected void CopyQLXXFromXZ(String qlid, BDCS_DJDY_GZ djdy, String sfnewcqr) {
		...
		if (bdcs_ql_xz != null) {
			...
			bdcs_ql_gz.setISCANCEL("1");//变更前的设置该权利标书，证书获取时候排除该权利
			//bdcs_ql_gz.setBDCQZH("");// 重置不动产权证明号 赞不重置
			...
			if (bdcs_ql_xz.getFSQLID() != null) {
				...
				if (bdcs_fsql_xz != null) {
					...
					//if (!"1".equals(sfnewcqr)) {
					//	bdcs_fsql_gz.setDYR("");// 重置抵押人 赞不重置赞不重置
					//}
			...
			if (qlrs != null && qlrs.size() > 0) {
				for (int iqlr = 0; iqlr < qlrs.size(); iqlr++) {
					...
					if (bdcs_qlr_xz != null) {
						...
						bdcs_qlr_gz.setXMBH(getXMBH());// 重置项目编号
						//bdcs_qlr_gz.setBDCQZH("");// 重置不动产权证明号 赞不重置
						//bdcs_qlr_gz.setSQRID(sqr.getId());// 重置申请人id
						getCommonDao().save(bdcs_qlr_gz);
						// 获取证书集合
				/*		StringBuilder builder = new StringBuilder();
