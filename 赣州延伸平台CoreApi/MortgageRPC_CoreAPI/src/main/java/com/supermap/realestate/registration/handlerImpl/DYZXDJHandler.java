 ...
 */
public class DYZXDJHandler extends DJHandlerBase implements DJHandler {
	 ...
	 * @param info
	 */
	public DYZXDJHandler(ProjectInfo info) {
		super(info);
	}
		...
		if (qlid != null && qlid.length > 0) {
			for (String id : qlid) {
				...
				if (bdcs_ql_xz != null) {
					...
					// 获取所有权人
					String ywrmc = "";
					String syqsqlcondition = builderDJDY.toString() + " and QLLX IN ('1','2','3','4','5','6','7','8','15','99')";
					...
					if (rightss != null && rightss.size() > 0) {
						...
						//在建工程抵押注销————没有主体权利的情况下
						syqsqlcondition = builderDJDY.toString();
						rightss = RightsTools.loadRightsByCondition(DJDYLY.XZ, syqsqlcondition);
						if (rightss != null && rightss.size() == 1) {
							if("23".equals(rightss.get(0).getQLLX())){
								String[] dyrs = bdcs_fsql_xz.getDYR().split(",");
								List<BDCS_SQR> ywrs =  getCommonDao().getDataList(BDCS_SQR.class, "XMBH='"+bdcs_ql_xz.getXMBH() +"' AND SQRLB='2'");
								for (BDCS_SQR ywr:ywrs) {
									for (String dyr:dyrs) {
										if (dyr.equals(ywr.getSQRXM())) {
											BDCS_SQR sqr = new BDCS_SQR();
											ObjectHelper.copyObject(ywr, sqr);
											String sqrId=SuperHelper.GeneratePrimaryKey();
											sqr.setId(sqrId);												
											sqr.setXMBH(getXMBH());
											sqr.setGLQLID(bdcs_ql_gz.getId());
											getCommonDao().update(sqr);
											getCommonDao().flush();
										}
									}
								}
		...
		try {
			...
			if(!StringHelper.isEmpty(rights) && rights.size()>0){
				...
				for(Rights right :rights){
					...
					sfList = packageXml.getDJSFEx(new DJFDJSF(), ywh, this.getXMBH(), unit);
...
//					msg.getData().setDJSF(sfList);
					...
					if(StringHelper.isEmpty(zddm)&&unit!=null&&!StringHelper.isEmpty(unit.getBDCDYH())&&unit.getBDCDYH().length()>=19) {
							zddm=unit.getBDCDYH().substring(0, 19);
					}
