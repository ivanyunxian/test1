...
/**
 * 房地一体解封登记处理类
 * @ClassName: CFDJ_ZX_HouseAndLandHandler
 * @author liangc
 * @date 2018年5月14日 下午10:03:30
 */
public class CFDJ_ZX_HouseAndLandHandler extends DJHandlerBase implements DJHandler {

	public CFDJ_ZX_HouseAndLandHandler(ProjectInfo info) {
		super(info);
	}
		...
		try {
			...
			if (djdys != null && djdys.size() > 0) {
				...
				for (int i = 0; i < djdys.size(); i++) {
					...
					if (djdy != null&&BDCDYLX.H.Value.equals(djdy.getBDCDYLX())) {
...
//				bdcql.setId(lyqlid);
	 ...
	 * 
	 * @Title: addZDBDCDY   
	 * @Description: 添加土地bdcdy  
	 * @author liangJ
	 * @param: @param h_bdcdyid      
	 * @return: void      
	 * @throws
	 */
	public void addZDBDCDY(String h_bdcdyid){
		
		CommonDao dao = this.getCommonDao();
		BDCS_H_XZ h_XZ = getCommonDao().get(BDCS_H_XZ.class, h_bdcdyid);
		if(h_XZ==null||StringHelper.isEmpty(h_XZ.getZDBDCDYID())) {
			return ;
		}
		String  zdbdcdyid = h_XZ.getZDBDCDYID();
		String  djdySql = " djdyid in(select djdyid from bdck.bdcs_djdy_xz where  bdcdyid='"+zdbdcdyid+"' and bdcdylx='02') and qllx='99'";
		
		List<BDCS_QL_XZ> xzqls = dao.getDataList(BDCS_QL_XZ.class, djdySql);
		if(xzqls!=null&&xzqls.size()>0) {
			for (BDCS_QL_XZ bdcs_QL_XZ : xzqls) {
				String qlid = bdcs_QL_XZ.getId();
				BDCS_QL_XZ xz_ql = bdcs_QL_XZ;
				BDCS_FSQL_XZ xz_fsql = getFSQLXZbyQLID(dao, qlid);
				BDCS_DJDY_XZ xz_djdy = getDJDYXZbyDJDYID(dao, xz_ql.getDJDYID());
...
//					}
	 ...
	 * @throws
	 */
	public void removeZDBDCDY(String h_bdcdyid) {
		...
		CommonDao baseCommonDao = this.getCommonDao();
		BDCS_DJDY_GZ djdy = super.removeDJDY(zdbdcdyid);
		if (djdy != null) {
			String djdyid = djdy.getDJDYID();
			// 删除登记单元索引
			baseCommonDao.deleteEntity(djdy);
