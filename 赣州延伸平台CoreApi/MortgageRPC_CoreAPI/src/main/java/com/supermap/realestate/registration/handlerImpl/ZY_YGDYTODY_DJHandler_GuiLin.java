...
/**
* 
* 转移登记_国有建设用地使用权/房屋所有权+现房抵押预告预告登记转现房抵押登记操作类
* @ClassName: ZY_YGDYTODY_DJHandler_GuiLin 
* @author liangc 
* @date 2017年9月13日 下午04:19:30
*/
public class ZY_YGDYTODY_DJHandler_GuiLin extends DJHandlerBase implements DJHandler {

	/**
	 * 构造函数
	 * @param info
	 */
	public ZY_YGDYTODY_DJHandler_GuiLin(ProjectInfo info) {
		super(info);
	}
	...
	public boolean addBDCDY(String bdcdyid) {
		...
		if (djdy != null) {
			...
			builer.append("' ");
			builer.append("WHERE DYQL.QLLX='23' AND DYQL.DJLX='700'");
			...
			if (listDYQ != null && listDYQ.size() > 0) {
				for (int dyqqlid = 0; dyqqlid < listDYQ.size(); dyqqlid++) {
					CopyQLXXFromYDY(StringHelper.formatObject(listDYQ.get(dyqqlid).get("QLID")),djdy);
				}
			}else{
				//生成抵押权
			...
			// 将预抵押的权利人拷贝到申请人
			String fulsql=" SELECT QLID FROM BDCK.BDCS_QL_XZ QL LEFT JOIN BDCK.BDCS_DJDY_XZ DJDY ON QL.DJDYID=DJDY.DJDYID WHERE DJDY.BDCDYID='"+bdcdyid+"' AND QL.QLLX='23' AND QL.DJLX='700'";
		...
		if (djdys != null && djdys.size() > 0) {
			for (int idjdy = 0; idjdy < djdys.size(); idjdy++) {
				...
				if (bdcs_djdy_gz != null) {
					...
					//删除预告登记权利
					super.deletePreHouseRights(bdcdyid,"9","700","因办理转移登记+预抵押转现业务，自动注销");
					//删除预抵押登记权利
