 ...
 */
public class DYZXCD_DJHandler extends DJHandlerBase implements DJHandler {
	 ...
	 * @param info
	 */
	public DYZXCD_DJHandler(ProjectInfo info) {
		super(info);
	}
	...
	public boolean addBDCDY(String qlids) {
		...
		if (qlid != null && qlid.length > 0) {
			for (String id : qlid) {
				...
				if (ql != null) {
					// 拷贝抵押权
					BDCS_QL_GZ old_bdcs_ql_gz = new BDCS_QL_GZ();
					try {
						PropertyUtils.copyProperties(old_bdcs_ql_gz,ql);
...
//					old_bdcs_ql_gz.setYWH(this.getProject_id());
//					old_bdcs_ql_gz.setXMBH(this.getXMBH());
//					old_bdcs_ql_gz.setISCANCEL("1");
//					
					// 拷贝登记单元
	...
	public boolean writeDJB() {
		...
		SubRights newright = null;
		SubRights tmp = null;
		for (Rights right : lstrights) {
			// 根据权利ID删除权利，附属权利，证书，权地证人，权利人信息
			Rights old_right=RightsTools.deleteRightsAll(DJDYLY.XZ, right.getLYQLID());
			commonDao.deleteEntity(old_right);
			// 更新历史附属权利信息
			subright = RightsTools.loadSubRightsByRightsID(DJDYLY.LS, right.getLYQLID());
			newright = RightsTools.loadSubRightsByRightsID(DJDYLY.GZ, right.getId());
			tmp = newright;
			if (subright != null) {
				Date zxsj = new Date();
				...
				yx=newright.getZXDYYY();
				subright.setZXDYYY(yx);
				fj=newright.getZXFJ();
				subright.setZXFJ(fj);
				newright.setZXDYYY("");
				newright.setZXFJ("");
				newright.setZXDYYWH("");
				commonDao.update(newright);
