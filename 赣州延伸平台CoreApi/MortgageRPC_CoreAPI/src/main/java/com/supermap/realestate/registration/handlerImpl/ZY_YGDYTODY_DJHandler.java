...
/*
1、房屋转移+现房抵押预告登记转现房抵押登记
*/
/**
* 
* 转移登记+现房抵押预告登记转抵押登记
* @ClassName: ZY_YGDYTODY_DJHandler 
* @author 俞学斌 
* @date 2016年01月28日 下午05:20:16
*/
public class ZY_YGDYTODY_DJHandler extends DJHandlerBase implements DJHandler {

	/**
	 * 构造函数
	 * @param info
	 */
	public ZY_YGDYTODY_DJHandler(ProjectInfo info) {
		super(info);
	}
	...
	public boolean addBDCDY(String bdcdyid) {
		...
		if (djdy != null) {
			...
			List<Rights> dyygqls=RightsTools.loadRightsByCondition(DJDYLY.XZ, " QLLX='23' AND DJLX='700' AND DJDYID='"+djdy.getDJDYID()+"'");
			if(dyygqls!=null&&dyygqls.size()>0){
				for(Rights dyygql:dyygqls){
					CopyQLXXFromYGDY(dyygql,djdy);
				}
			}else{
				//生成抵押权
				...
				dyfsql.setYGDJZL(YGDJLX.QTBDCMMYGDJ.Value.toString());
				dao.save(dyql);
				dao.save(dyfsql);
			}
	...
	protected void CopyQLXXFromYGDY(Rights ql,BDCS_DJDY_GZ djdy) {
		...
		BDCS_QL_XZ bdcs_ql_xz=(BDCS_QL_XZ)ql;
		// 拷贝权利
		...
		bdcs_ql_gz.setLYQLID(ql.getId());//设定来源权利ID
		...
		// 获取权利人集合
		List<BDCS_QLR_XZ> qlrs = getCommonDao().getDataList(BDCS_QLR_XZ.class, " QLID='"+ql.getId()+"'");
			...
			for (int iqlr = 0; iqlr < qlrs.size(); iqlr++) {
				...
				if (bdcs_qlr_xz != null) {
					...
					if ("1".equals(isextendmortgageinfo)) {
						...
						builder.append(ql.getId()).append("'").append(" AND QLRID='");
	...
	private void ZXYGDYDJ(String djdyid){
		String dbr = Global.getCurrentUserName();
	...
	public void addQLRbySQRArray(String qlid, Object[] sqrids) {
		...
		if(ql != null && !StringHelper.isEmpty(ql) && !ql.getQLLX().equals(QLLX.DIYQ.Value)){
			String xmbhFilter=ProjectHelper.GetXMBHCondition(getXMBH());
