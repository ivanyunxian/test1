...
@Service("agricultureQueryService")
public class agricultureQueryServiceImpl implements agricultureQueryService {
	@Autowired
	private CommonDao baseCommonDao;
	
	@SuppressWarnings({ "rawtypes", "unchecked" })
	@Override
	public Message queryAgriculture(Map<String, String> queryvalues, int page, int rows, boolean iflike, String sort, String order) {
		...
		/* ===============1、先获取实体对应的表名=================== */
		String unitentityName = "BDCK.BDCS_NYD_XZ";

		/* ===============2、再获取表名+'_'+字段名=================== */
		String dyfieldsname = "DJDY.DJDYID,DY.BDCDYID,DY.BDCDYH,DY.ZL,'09' AS BDCDYLX ,'农用地' AS BDCDYLXMC ,DY.CBMJ ";
					...
					if (iflike) {
						...
						if (name.equals("QLR.ZJH") && value.replace(" ", "").length() == 18) {
							...
							if (oldCard.length() != 15) {
								...
								qlrbuilder.append(" (" + name + "='" + value.replace(" ", "").toUpperCase() + "' or ");
	...
	@SuppressWarnings({ "unchecked", "rawtypes" })
	@Override
	public List<Map> getAgricultureInfo(String bdcdyid, String bdcdylx) {
		...
		if (unit != null && BDCDYLX.NYD.equals(BDCDYLX.initFrom(bdcdylx))) {
			list.add(beanToMap(unit));
		}
		if (list != null && list.size() > 0) {
			String cbqssj = StringHelper.FormatDateOnType(list.get(0).get("cbqssj"), "yyyy-MM-dd");
			String cbjssj = StringHelper.FormatDateOnType(list.get(0).get("cbjssj"), "yyyy-MM-dd");
			list.get(0).put("cbqssj", cbqssj);
			list.get(0).put("cbjssj", cbjssj);
		}
		return list;
	}
	...
	private Rights getQLInfo_XZ(String bdcdyid) {
		...
		if (djdys != null && djdys.size() > 0) {
			...
			if (BDCDYLX.NYD.equals(lx)) {
				List<Rights> qls = RightsTools.loadRightsByCondition(DJDYLY.XZ, "DJDYID='" + djdyid + "' and QLLX IN ('24')");
		...
		if (lsqls != null && lsqls.size() > 0) {
			for (int iql = 0; iql < lsqls.size(); iql++) {
				...
				String fulSql = "SELECT DISTINCT PRO.PRODEF_NAME,BASE.DJLX FROM BDCK.BDCS_QL_LS LSQL INNER JOIN BDC_WORKFLOW.WFI_PROINST PRO ON PRO.FILE_NUMBER=LSQL.YWH "
						...
						+ " AND (BASE.DJLX='500' OR BASE.DJLX='300') AND BASE.QLLX='23' AND BASE.UNITTYPE='09' AND LSQL.QLID=''{0}''";
	...
	private String GetYWLX(String djlx, String qllx, BDCDYLX dylx) {
		...
		if (DJLX.CFDJ.Value.equals(djlx) || DJLX.YYDJ.Value.equals(djlx)) {
			...
			if (DJLX.YGDJ.Value.equals(djlx) && "99".equals(qllx)) {
				...
				ywlx = "抵押预告登记";
			} else if (QLLX.GYNYDSHYQ.Value.equals(qllx)) {
