 ...
 *
 */
public class BG_YGandDYQBG_DJHandler extends DJHandlerBase implements DJHandler {
	 ...
	 * @param info
	 */
	public BG_YGandDYQBG_DJHandler(ProjectInfo info) {
		super(info);
	}
...
//			}
			...
			//流程GZ017，拷贝上一手预告登记的义务人
			if ("GZ017".equals(((WFD_MAPPING)listCode.get(0)).getWORKFLOWNAME())) {
				String xmbh= (getCommonDao().getDataList(BDCS_DJDY_XZ.class, MessageFormat.format(" BDCDYID=''{0}'' ", bdcdyid))).get(0).getXMBH();
				...
				for (int iywr = 0; iywr < sqrlist.size(); iywr++) {
					...
					sqr_ywr.setGLQLID(bdcs_ql_gz.getId());
					sqr_ywr.setId(SQRID);
			...
			if("450300".equals(xzqdm)){
				List<String> xmbhLists = new ArrayList<String>();
				for(BDCS_QL_XZ ql : list){					
					if(DJLX.YGDJ.Value.equals(ql.getDJLX()) && !StringHelper.isEmpty(ql.getXMBH()) && 
							!xmbhLists.contains(ql.getXMBH())){
	...
	public boolean removeBDCDY(String bdcdyid) {
		...
		if (djdy != null) {
			...
			//删除义务人
			List<BDCS_SQR> ywrLists = baseCommonDao.getDataList(BDCS_SQR.class, "XMBH='"+getXMBH()+"' AND SQRLB='"+SQRLB.YF.Value+"'");
			if(ywrLists != null && ywrLists.size() >0){
				for(BDCS_SQR ywr : ywrLists){
					baseCommonDao.deleteEntity(ywr);
				}
			}
	...
	private void addQLRbySQRs_NOTOPERTATEYWR(String qlid, Object[] sqrids) {
		...
		if (sqrids != null && sqrids.length > 0) {
			...
			//桂林需求-把义务人添加到fsql的ywr字段
			if("450300".equals(ConfigHelper.getNameByValue("XZQHDM")))
			if ((DJLX.YGDJ.equals(this.getDjlx())||DJLX.YGDJ.Value.equals(ql.getDJLX())||DJLX.GZDJ.Value.equals(ql.getDJLX())) && 
					!QLLX.QTQL.equals(this.getQllx()) && !QLLX.DIYQ.Value.equals(ql.getQLLX())) {
