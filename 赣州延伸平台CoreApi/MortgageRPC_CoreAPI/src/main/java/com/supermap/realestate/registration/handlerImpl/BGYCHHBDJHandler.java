 ...
 */
public class BGYCHHBDJHandler extends DJHandlerBase implements DJHandler {
	 ...
	 * @param info
	 */
	public BGYCHHBDJHandler(ProjectInfo info) {
		super(info);
	}
	...
	public boolean addBDCDY(String bdcdyid) {
		...
		for (int i = 0; i < ids.length; i++) {
			...
			long count = dao.getCountByFullSql(" FROM BDCK.BDCS_H_XZY WHERE BDCDYID='"+ id +"'");
//			if(count<=0){
//				return false;
//			}
		}
			...
			if (dy != null) {// 登记单元索引表
			  ...
			  RealUnit dy_dc =UnitTools.loadUnit(this.getBdcdylx(), DJDYLY.DC, bdcdyid);
				if(!StringHelper.isEmpty(dy_dc))
				{
				   ...
				   House h=(House) dy;
				   if(StringHelper.isEmpty(bghzrz)){
					   bghzrz=h.getZRZBDCDYID();
				   }else{
					   if(!bghzrz.equals(h.getZRZBDCDYID())){
						   return false;
					   }
				   }
				}
