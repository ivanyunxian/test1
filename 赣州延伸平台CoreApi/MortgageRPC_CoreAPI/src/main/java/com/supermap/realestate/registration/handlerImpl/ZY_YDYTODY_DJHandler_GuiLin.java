...
*/
public class ZY_YDYTODY_DJHandler_GuiLin extends DJHandlerBase implements DJHandler {

	/**
	 * 构造函数
	 * @param info
	 */
	public ZY_YDYTODY_DJHandler_GuiLin(ProjectInfo info) {
		super(info);
	}
		...
		if (bdcs_ql_xz != null) {
			...
			if (bdcs_ql_xz.getFSQLID() != null) {
				...
				if (bdcs_fsql_xz != null) {
					...
					bdcs_fsql_gz.setDYWLX(getDYBDCLXfromBDCDYLX(getBdcdylx()));//重置抵押物类型
					bdcs_fsql_gz.setDYBDCLX(getDYBDCLXfromBDCDYLX(getBdcdylx()));//重置抵押不动产类型
					if ("0".equals(isextendmortgageinfo)) {	
