...
public class ConverIdCard {
	...
	// 判断串长度的合法性
	private static boolean GetIsNew(String idCar) {
		boolean isNew = false;
		if (!StringHelper.isEmpty(idCar) && idCar.length() == newIDLen) {
			isNew = true;
		}
		return isNew;
	}
	...
	public static String getOldIDCard(final String newIDCard) {
		...
		String oldIDCard = newIDCard.substring(0, fPart) + newIDCard.substring(fPart + yearFlag.length(), newIDCard.length() - 1);
		return oldIDCard;
	}
	...
	public static void main(String[] args) {
		...
		String randomID = ConverIdCard.getRandomIDCard(true);
		System.out.println("随机身份证：" + randomID);
		/**//*
			 * String oldID=""; String newID=Identity.getNewIDCard(oldID);
			 * System.out.println("旧身份证："+oldID);
			 * System.out.println("新身份证："+newID); String oldCard =
			 * Identity.getOldIDCard(newID);
			 * System.out.println("旧身份证："+oldCard); /* String
			 * dateSource="2000-9-30"; if(id.checkDate(dateSource))
			 * System.out.println("正确时间串："+dateSource); else
