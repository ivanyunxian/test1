...
/**
 * 房地一体登记处理类
 * @ClassName: CFDJHouseAndLandJf
 * @author zql
 * @date 2015年9月8日 下午10:22:41
 */
public class CFDJHouseAndLandJf extends DJHandlerBase implements DJHandler {

	public CFDJHouseAndLandJf(ProjectInfo info) {
		super(info);
	}
		...
		if (djdys != null && djdys.size() > 0) {
			for (int idjdy = 0; idjdy < djdys.size(); idjdy++) {
				...
				if (bdcs_djdy_gz != null) {
					...
					updateZD(djdyid,bdcdyid,dbr,xmbh);
				}
			}
		}
		this.SetSFDB();
...
//				bdcql.setId(lyqlid);
	...
	//List<Rights> list_ql_zd_old=RightsTools.loadRightsByCondition(DJDYLY.XZ, "QLLX IN ('99') AND DJDYID='"+djdyid_zd+"' AND XMBH='"+xmbh+"'");
		List<Rights> list_ql_zd_old=RightsTools.loadRightsByCondition(DJDYLY.XZ, "QLLX IN ('99')  AND DJLX='800' AND DJDYID='"+djdyid_zd+"'");
