...
/*
 1、国有建设用地使用权/房屋所有权转移登记
 2、集体建设用地使用权/房屋所有权转移登记
 3、宅基地使用权/房屋所有权转移登记
 */
/**
 * 
 * 转移登记处理类(房地一起转移)
 * 
 * @ClassName: ZYDJHandler_HouseAndLand
 * @author yuxuebin
 * @date 2016年05月10日 14:43:29
 */
public class ZYDJHandler_HouseAndLand extends DJHandlerBase implements
		DJHandler {
	 ...
	 * @param info
	 */
	public ZYDJHandler_HouseAndLand(ProjectInfo info) {
		super(info);
	}
	...
	public boolean addBDCDY(String bdcdyid) {
		...
		if (djdy != null) {
			...
			dao.save(djdy);
			// 拷贝转移前权利人到申请人
			super.CopySQRFromXZQLR(djdy.getDJDYID(), ql.getQLLX(),
					this.getXMBH(), ql.getId(), SQRLB.YF.Value);
	...
	private boolean IsCanAccept(String bdcdyid){
		...
		if(!(XZQHDM.indexOf("45") == 0 )){  //广西区县--房地一起的转移登记（ZY020、ZY021、ZY022)，需在地与房的权利人不一致的情况下也能转移。
	...
	protected boolean CopyZYYGQLRToSQR(String qlid, String djdyid) {
		List<Rights> zyyg_qls = RightsTools.loadRightsByCondition(DJDYLY.XZ,
				"DJDYID='" + djdyid + "' AND DJLX='700' AND QLLX='99'");
