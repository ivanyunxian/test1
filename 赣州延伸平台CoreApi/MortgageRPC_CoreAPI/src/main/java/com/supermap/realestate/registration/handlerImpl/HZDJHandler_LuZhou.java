 ...
 */
/**
 * 换证登记处理类泸州版本（拷贝户中的土地起始时间跟结束时间）
 * @author 海豹
 *
 */
public class HZDJHandler_LuZhou extends DJHandlerBase implements DJHandler {
	 ...
	 * @param info
	 */
	public HZDJHandler_LuZhou(ProjectInfo info) {
		super(info);
	}
	...
	public boolean addBDCDY(String bdcdyid) {
		...
		RealUnit unit = null;
		try {
			unit = UnitTools.loadUnit(this.getBdcdylx(), DJDYLY.XZ, bdcdyid);
		} catch (Exception e) {
			e.printStackTrace();
		}
		// 拷贝登记单元
		...
		if (qls != null && qls.size() > 0) {
			...
			if (unit instanceof House) {
				...
				if(bdcs_ql_gz !=null)
				{
				bdcs_ql_gz.setQLQSSJ(house.getTDSYQQSRQ());
				bdcs_ql_gz.setQLJSSJ(house.getTDSYQZZRQ());
