...
public class SmLand {
	 ...
	 *            
	 * */
	public Message LandList(int pageIndex, int pageSize, String ps_name,String new_ps_name,String hs_buildarea, String pa_location, String re_landcertno, String pa_area) {
		...
		if (new_ps_name != null && !new_ps_name.equals("")) {
			if(nowhereString.length()>0)
			{
				nowhereString.append(" AND ");
			}
			nowhereString.append("NEW_PS_NAME LIKE '%");
			nowhereString.append(new_ps_name.replace(" ", "%"));
			nowhereString.append("%'");
		}
		...
		fromSql.append(" FROM BDCK.BDCS_LANDCERT T1");
