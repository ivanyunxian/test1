package com.supermap.realestate_gx.registration.common;
...
/**
 * @author 包基杰 桂林公积金中间库公用dao
 * @action 一个通用的Hibernate的dao
 *
 */
@Repository("accumufundDao")
public class CommonDao {

	public static int icount = 0;
	@Autowired
//	@Qualifier("sessionFactoryAccumufund")
	private SessionFactory sessionFactoryAccumufund;

	private Session getCurrentSession() {
	   Session s = null;
	   try{
		   s=this.sessionFactoryAccumufund.getCurrentSession();
	   }catch(Exception e){
		   s= this.sessionFactoryAccumufund.openSession();
	   }
	   return s;
}
	...
	public Boolean ExecuteOraProc(String ProcString, ArrayList<String> paralist) {
		...
		try {
			statement = SessionFactoryUtils.getDataSource(this.sessionFactoryAccumufund).getConnection().prepareCall(str.toString());
			for (int i = 0; i < paralist.size(); i++) {
	...
	/**
	 * 根据完整SQL语句一次新查询所有数据，参数化查询，分页查询
	 * @作者 凌广清
	 * @创建时间 2018年6月12日 09点44分
	 * @param fulSql
	 * @param paramMap
	 * @param pageIndex
	 * @param pageSize
	 ...
	 */
	@SuppressWarnings("rawtypes")
	public List<Map> getDataListByFullSql(String fulSql, Map<String, String> paramMap, int pageIndex, int pageSize) {
		Query query = this.getCurrentSession().createSQLQuery(fulSql).setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP);
		query.setProperties(paramMap);
