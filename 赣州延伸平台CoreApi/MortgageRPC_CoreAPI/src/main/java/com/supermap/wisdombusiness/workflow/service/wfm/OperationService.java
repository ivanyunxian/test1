...
import com.supermap.realestate.registration.service.impl.DXSendServicelmpl;
...
import com.supermap.wisdombusiness.workflow.model.RT_USERROLE;
import com.supermap.wisdombusiness.workflow.model.T_ROLE;
import com.supermap.wisdombusiness.workflow.model.T_ROLEGROUP;
...
import com.supermap.wisdombusiness.workflow.model.Wfi_DbList;
...
public class OperationService {
	...
	@Autowired
	private DXSendServicelmpl dxSendServicelmpl;
	@Autowired
	...
	public SmObjInfo PassOver(String routeid, String actinstid, List<SmObjInfo> staffids, String staffid, String Msg, boolean more) {
		final SmObjInfo smObjInfomsg = new SmObjInfo();
		...
		// 调用转出
		List<Wfi_ActInst> actidlist = smActInst.PassOver(staffid, routeid, actinstid, staffs, Msg);
		actidlist = autoPassOver(actidlist,staffid, staffs, Msg);
		...
		if (actidlist != null && actidlist.size() > 0) {
			for (Wfi_ActInst wfi_ActInst : actidlist) {
				...
				sendProject(instid);
				//是否自动锁定
				if(null!=smActInst.GetActDef(instid).getAutoLock()&&smActInst.GetActDef(instid).getAutoLock()>0){
			...
			String routeName=routedefList.get(0).getRoute_Name();
			...
			if(SENDMESSAG.equals("1")){
				...
				//TODO 先过滤玉林的 
				if (!XZQHDM.contains("4509")) {
					if ("到登簿".contains(routeName)||"到缮证".contains(routeName)&& ""!=routeName ) {
						String	actinstSql="select * from bdc_workflow.wfi_actinst t where t.actinst_id='"+actinstid+"'";
						List<Wfi_ActInst> proinst_id=commonDao.getDataList(Wfi_ActInst.class,actinstSql);
						String proinstId=StringHelper.FormatByDatatype(proinst_id.get(0).getProinst_Id());

						String	wfiSql="select * from bdc_workflow.wfi_proinst t where t.proinst_id='"+proinstId+"'";
						List<Wfi_ProInst> file_Number=commonDao.getDataList(Wfi_ProInst.class,wfiSql);
						String fileNumber=StringHelper.FormatByDatatype(file_Number.get(0).getFile_Number());

						String	xmxxSql="select * from bdck.bdcs_xmxx t where t.project_id='"+fileNumber+"'";
						List<BDCS_XMXX> xmxxbh=commonDao.getDataList(BDCS_XMXX.class,xmxxSql);
						final String xmbh=StringHelper.FormatByDatatype(xmxxbh.get(0).getId());

						String	sqrSql="select * from bdck.bdcs_sqr t where t.xmbh='"+xmbh+"'";
						List<BDCS_SQR> sqr=commonDao.getDataList(BDCS_SQR.class,sqrSql);
						final String sqrid=StringHelper.FormatByDatatype(sqr.get(0).getId());

						if (null !=xmbh && null!=sqrid) {
							taskExecutor.execute(new Runnable() {
								@Override
								public void run() {
									try {
										Message msg=new Message();
										msg= dxSendServicelmpl.dtdxsend(xmbh, sqrid,"1");
	 ...
	 * @return
	 */
	private List<Wfi_ActInst> autoPassOver(List<Wfi_ActInst> actidlist, String staffid, List<SmObjInfo> staffs, String msg){
		try{
			for(Wfi_ActInst actInst : actidlist) {
				Wfd_Actdef actdef = commonDao.get(Wfd_Actdef.class,	actInst.getActdef_Id());
				if(!StringHelper.isEmpty(actdef) && !"1".equals(StringHelper.formatObject(actdef.getIsautopass()))) {
					continue;
				}
				List<Wfd_Route> wfd_routes = smRouteDef.GetAllRoutesByActdef(actdef);
				//获取活动对应的办理人员
				List<SmObjInfo> infos = smStaff.GetActStaffByActInst(actInst.getActinst_Id());
				List<SmObjInfo> actinstSmobj = infos.get(0).getChildren();
				if(wfd_routes.size()>1) {
					//并发路由的话有多个，循环吧---关键是并发和单路由取值不一样，真是*了狗了
					for(Wfd_Route route : wfd_routes) {
						for(SmObjInfo smObjInfo : actinstSmobj) {
							if(route.getRoute_Id().equals(smObjInfo.getRouteid())) {
								continue;
							}
							List<SmObjInfo> depts = smObjInfo.getChildren();
							//拿第一个部门的人员来搞
							List<SmObjInfo> users = depts.get(0).getChildren();
							String userid = users.get(0).getID();
							List<Wfi_ActInst> nex_actidlist = smActInst.PassOver(userid, route.getRoute_Id(), actInst.getActinst_Id(), users, msg);
							//递归吧
							actidlist = autoPassOver(nex_actidlist, userid, users, msg);
						}
					}
				} else {
					for(Wfd_Route route : wfd_routes) {
						for(SmObjInfo smObjInfo : actinstSmobj) {
							if(route.getNext_Actdef_Id().equals(smObjInfo.getRouteid())) {
								continue;
							}
							List<SmObjInfo> depts = smObjInfo.getChildren();
							//拿第一个部门的人员来搞
	...
	private void sendProject(String newactinstid) {
		Wfd_Actdef actdef = smActInst.GetActDef(newactinstid);
		if (actdef != null) {
...
//		System.out.println("1");
		...
		if (actinstid != null && !actinstid.equals("")) {
			try {
				...
				Wfi_ActInst actinset = smActInst.GetActInst(actinstid);
				...
				String XZQHDM=ConfigHelper.getNameByValue("XZQHDM");
				if(SENDMESSAG.equals("1") || XZQHDM.contains("4509")){
					...
					if(actinset.getActinst_Name().contains(ac_name)){
						try {
							//玉林 使用本地短信平台
							if (XZQHDM.contains("4509")) {
								smProInstService.SmsSend(inst.getFile_Number());
							} else {
...
//						System.out.println("5");
	...
	public SmObjInfo DirectOverRule(String actinstid) {
		...
		try {
			...
			if (smActDef.IsStartAct(actdef) && (!"1".equals(StringHelper.FormatByDatatype(prodef.getHouse_Status())) && StringHelper.isEmpty(proinst.getWLSH()))) {// 是否起始活动
				smObjInfo.setID(WFConst.SmActInstOverRuleReturnType.CannotOverRuleStartAct.value + "");
				smObjInfo.setDesc("初始活动不允许驳回");
//			}else if(smActDef.IsStartAct(actdef) && "1".equals(StringHelper.FormatByDatatype(prodef.getHouse_Status())) && !StringHelper.isEmpty(proinst.getWLSH())){
			}else if(actdef.getActdef_Name().contains("登簿") && "1".equals(StringHelper.FormatByDatatype(prodef.getHouse_Status())) && !StringHelper.isEmpty(proinst.getWLSH())){
				smObjInfo.setID(proinst.getWLSH());//使用外网生成的流水号关联数据
				smObjInfo.setFile_number(proinst.getFile_Number());
			}else if(smActDef.IsStartAct(actdef) && "1".equals(StringHelper.FormatByDatatype(prodef.getHouse_Status())) && StringHelper.isEmpty(proinst.getWLSH())) {
				smObjInfo.setID(WFConst.SmActInstOverRuleReturnType.CannotOverRuleStartAct.value + "");
				smObjInfo.setDesc("内网创建受理的智能审批流程不允许驳回外网");
			}else if(smActDef.IsStartAct(actdef)  && !StringHelper.isEmpty(proinst.getWLSH()) && ("".equals(StringHelper.FormatByDatatype(prodef.getHouse_Status())) || "0".equals(StringHelper.FormatByDatatype(prodef.getHouse_Status())))) {
				//上面不用 || 来判断，主要是为了防止在内网使用智能审批流程，也驳回到外网
				smObjInfo.setID(proinst.getWLSH());//使用外网生成的流水号关联数据
				smObjInfo.setFile_number(proinst.getFile_Number());
			}
	...
	public SmObjInfo OverRuleDirectly(String proactdefid, String actinstid, List<SmObjInfo> Actinfo, String reson) {
		return smActInst.PassBack(proactdefid, actinstid, Actinfo, reson);
	...
	public SmObjInfo TurnProjectByactinstid(String actinstid, String staffid, String staffname) {
		...
		if (actInst != null) {
			...
			if (passwork != null && passwork.size()>0) {
				Wfd_Actdef def = smActInst.GetActDef(actinstid);
	...
	public List<SmObjInfo> passbackmsg(String actinstid) {
		...
		if (inst != null) {
			...
			if (inst.getOperation_Type() != null && inst.getOperation_Type().equals(WFConst.Operate_Type.PRODUCT_anglesPicture.value + "")) {
				...
				SmObjInfo info = new SmObjInfo();
				List<Wfi_ActInst> backInsts = smActInst.GetProActInsts(actinstid);
/*				List<Wfi_ActInst> list = smActInst.getActInstForCommonActdef(actinstid);
				...
				if (backInsts != null && backInsts.size() > 0) {
					...
					if (backInst.getOperation_Type() != null && backInst.getActinst_Msg() != null && !backInst.getActinst_Msg().equals("转出附言") && !backInst.getActinst_Msg().equals("受理项目备注")&& !backInst.getActinst_Msg().equals("受理项目备注来自在线受理")&&!backInst.getActinst_Msg().equals("批量转出")) {
						...
						List<Wfi_ActInst> list = smActInst.getActInstForCommonActdef(backInst.getActinst_Id());
						String actdef = smActInst.GetActInst(actinstid).getActdef_Id();
						for(Wfi_ActInst act : list){
							List<Wfi_ActInst> wfi_actinsts = smActInst.GetProActInsts(act.getActinst_Id());
	...
	public List<SmObjInfo> PassBackStaff(String actinst_id) {
		Wfd_Actdef actdef = smActInst.GetActDef(actinst_id);
		if (actdef != null) {
	...
	public SmObjInfo ValidateWithDraw(String actinstid) {
		...
		SmObjInfo info = new SmObjInfo();
		Wfd_Actdef actdef = smActInst.GetActDef(actinstid);
		List<Wfi_ActInst> list=smActInst.GetNextActInstList(actinstid, actdef.getActdef_Id());
	...
	public SmObjInfo WithDraw(String actinstid) {
		...
		Wfd_Actdef actdef = smActInst.GetActDef(actinstid);
		Wfi_ActInst actinst = smActInst.GetActInst(actinstid);
		List<Wfi_ActInst> nextactinsts = smActInst.GetNextActInstList(actinstid, actdef.getActdef_Id());
		if (nextactinsts != null &&!nextactinsts.isEmpty()) {
				...
				for(Wfi_ActInst inst : nextactinsts){
					...
					commonDao.update(inst);
					Wfi_NowActInst nowActInst = smActInst.GetNowActinst(inst.getActinst_Id());
						commonDao.delete(nowActInst);
				}
				Wfi_NowActInst nowActinst = smActInst.CreatNowActInst(newInst);
			...
			if (insts != null && insts.size() > 0) {
				...
				if (inst != null) {
					...
					if (actinsts != null && actinsts.size() > 0) {
						...
						commonDao.update(actinst);
						Wfi_NowActInst nowact = smActInst.CreatNowActInst(actinst);
	 ...
	 * @param actinstid 活动id staffid  办件人员ID
	 * 
	 */
	...
	public void sendPostUrl(String actinst_id){
		Wfi_ActInst actinst = smActInst.GetActInst(actinst_id);
		String actinstid = actinst.getFromActinstid();
		Wfd_Actdef actDef = smActInst.GetActDef(actinstid);
		Wfi_ProInst proInst = smProInst.GetProInstByActInstId(actinstid);
		String url = actDef.getPassUrl();
		if(!StringHelper.isEmpty(url)){
			String param = "project_id="+proInst.getFile_Number()
					+"&lsh="+proInst.getProlsh()
					+"&actinstid="+actinst_id;
			String[] str = url.split("\\?");
			if(str.length>1){
				param += "&" + str[1];
			}
			try {
				HttpRequestTools.sendPost(str[0], param);
			} catch (Exception e) {
				// TODO: handle exception
			}
		}
